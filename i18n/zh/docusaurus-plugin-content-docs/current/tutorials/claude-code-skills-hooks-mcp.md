...
**安全边界：** Hook 命令使用你的用户环境和权限执行。这意味着一个写错的 Hook 能做的事情，和你在终端里手动输入一条命令能做的事情，是同一个级别的。在审查 Hook 配置时，把它当成一段在你机器上运行的脚本来对待，不要因为它在 Claude Code 里就降低审查标准。[Coding Agent 沙箱安全指南](/zh/docs/tutorials/coding-agent-sandbox-security/)进一步解释了为什么 Workspace 写入与宿主机信任会把影响范围延伸到 Agent 进程之外。
